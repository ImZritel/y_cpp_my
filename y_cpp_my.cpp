...
public:
    ...
    vector<Document> FindTopDocuments(const string& raw_query, DocumentStatus status = DocumentStatus::ACTUAL) const {
        ...
        sort(matched_documents.begin(), matched_documents.end(),
            [](const Document& lhs, const Document& rhs) {
                return (abs(lhs.relevance - rhs.relevance) > EPSILON && lhs.relevance > rhs.relevance) || 
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string& raw_query, int document_id) const {
        ...
        tuple<vector<string>, DocumentStatus> pwords_status;
        ...
        for (const string& word : query.minus_words) {
            if (word_to_document_freqs_.count(word) == 0) {
                ...
                if (word_to_document_freqs_.at(word).count(document_id) > 0) {
                    return tuple(pwords, documents_.at(document_id).status);
                }
            }
        }
        ...
        pwords_status = tuple(pwords, documents_.at(document_id).status);
        return pwords_status;
    }
